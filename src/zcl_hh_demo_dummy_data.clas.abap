CLASS zcl_hh_demo_dummy_data DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    CONSTANTS c_error                     TYPE zhh_position-price VALUE '37707'.
    CONSTANTS c_number_of_invoices        TYPE i                  VALUE 300.
    CONSTANTS c_days_back_from_today      TYPE i                  VALUE 365.
    CONSTANTS c_max_number_of_positions   TYPE i                  VALUE 3.
    CONSTANTS c_max_quantity_per_position TYPE i                  VALUE 5.

  PRIVATE SECTION.
    DATA mt_partner         TYPE TABLE OF zhh_partner.
    DATA mt_material        TYPE TABLE OF zhh_material.
    DATA mt_discount        TYPE TABLE OF zhh_discount.
    DATA mt_head            TYPE TABLE OF zhh_invoice.
    DATA mt_position        TYPE TABLE OF zhh_position.

    DATA mo_random_partner  TYPE REF TO zcl_hh_random.
    DATA mo_random_date     TYPE REF TO zcl_hh_random.
    DATA mo_random_position TYPE REF TO zcl_hh_random.
    DATA mo_random_material TYPE REF TO zcl_hh_random.
    DATA mo_random_quantity TYPE REF TO zcl_hh_random.

    METHODS create_partner.
    METHODS create_material.
    METHODS create_discount.

    METHODS create_invoice
      IMPORTING id_count TYPE i.

    METHODS create_head
      RETURNING VALUE(rs_result) TYPE zhh_invoice.

    METHODS create_position
      IMPORTING is_head TYPE zhh_invoice.

ENDCLASS.


CLASS zcl_hh_demo_dummy_data IMPLEMENTATION.
  ...
  METHOD create_discount.
    mt_discount = VALUE #( ( partner = '1000000000' material = 'F0003' discount = '10.00' )
    ...
    DELETE FROM zhh_discount.
    INSERT zhh_discount FROM TABLE @mt_discount.
  ENDMETHOD.

  METHOD create_head.
    DATA ld_document TYPE n LENGTH 8 VALUE 30000000.

    IF mo_random_partner IS INITIAL.
      mo_random_partner = NEW #( id_min = 1
                                 id_max = lines( mt_partner ) ).
      mo_random_date = NEW #( id_min = 1
                              id_max = c_days_back_from_today ).
    ENDIF.

    rs_result = VALUE #( document = ld_document + lines( mt_head )
  ...
  METHOD create_invoice.
    DO id_count TIMES.
      ...
      create_position( ls_head ).
    ENDDO.

    DELETE FROM zhh_invoice.
    INSERT zhh_invoice FROM TABLE @mt_head.
    DELETE FROM zhh_position.
    INSERT zhh_position FROM TABLE @mt_position.
  ENDMETHOD.

  METHOD create_material.
    mt_material = VALUE #( ( material       = 'F0001'
                             name           = 'Peanuts'
    ...
    DELETE FROM zhh_material.
    INSERT zhh_material FROM TABLE @mt_material.
  ENDMETHOD.
  ...
  METHOD create_partner.
    ...
    DELETE FROM zhh_partner.
    INSERT zhh_partner FROM TABLE @mt_partner.
  ENDMETHOD.

  METHOD create_position.
    IF mo_random_position IS INITIAL.
      mo_random_position = NEW #( id_min = 1
                                  id_max = c_max_number_of_positions ).
      mo_random_material = NEW #( id_min = 1
                                  id_max = lines( mt_material ) ).
      mo_random_quantity = NEW #( id_min = 1
                                  id_max = c_max_quantity_per_position ).
    ENDIF.
    ...
    DO mo_random_position->rand( ) TIMES.
      ...
      TRY.
          DATA(ld_discount) = mt_discount[ partner  = is_head-partner
                                           material = ls_material-material ]-discount.
        CATCH cx_sy_itab_line_not_found.
          ld_discount = 0.
      ENDTRY.

      DATA(ls_position) = VALUE zhh_position(
      ...
      TRY.
          SELECT SINGLE FROM zhh_discount
            FIELDS currency_conversion( amount             = @ls_position-price,
