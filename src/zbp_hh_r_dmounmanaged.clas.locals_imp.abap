CLASS lcl_data_buffer DEFINITION.
  PUBLIC SECTION.
    CLASS-DATA gt_create TYPE zif_hh_demo_rap_data_handler=>tt_data.
    CLASS-DATA gt_update TYPE zif_hh_demo_rap_data_handler=>tt_data.
    CLASS-DATA gt_delete TYPE zif_hh_demo_rap_data_handler=>tt_data.
ENDCLASS.
...
CLASS lhc_Unmanaged IMPLEMENTATION.
  ...
  METHOD create.
    INSERT LINES OF CORRESPONDING zif_hh_demo_rap_data_handler=>tt_data(
      entities MAPPING cdate = CreationDate text = Description )
           INTO TABLE lcl_data_buffer=>gt_create.
  ENDMETHOD.

  METHOD update.
    DATA(lo_data_handler) = zcl_hh_demo_rap_data_handler=>create_instance( ).
...
CLASS lsc_ZHH_R_DMOUNMANAGED DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZHH_R_DMOUNMANAGED IMPLEMENTATION.
  METHOD finalize.
  ENDMETHOD.
  ...
  METHOD save.
    ...
    CLEAR: lcl_data_buffer=>gt_create,
           lcl_data_buffer=>gt_update,
           lcl_data_buffer=>gt_delete.
  ENDMETHOD.
